...
<div class="bg-white border-2 border-black rounded-lg p-2 space-x-2">
    {#each pageLinks as pageLink}
        <a style="background-color: {pageNum == pageLink ? "rgb(248,113,113)" : ""}" href="{base}/puzzles?page={pageLink}" class="px-2 py-1 rounded-lg transition-colors hover:bg-gray-300">{pageLink}</a>
    {/each}
</div>
